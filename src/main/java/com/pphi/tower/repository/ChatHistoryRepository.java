...
public class ChatHistoryRepository {
    ...
    public void save(String reportId1, String reportId2, String role, String message) {
        jdbc.update(
                "INSERT INTO chat_history (report_id_1, report_id_2, role, message, created_at) VALUES (?, ?, ?, ?, ?)",
                reportId1, reportId2, role, message, System.currentTimeMillis());
    }

    public List<ConversationTurn> findByPair(String reportId1, String reportId2) {
        return jdbc.query(
                "SELECT role, message FROM chat_history WHERE report_id_1 = ? AND report_id_2 = ? ORDER BY id ASC",
                (rs, rowNum) -> new ConversationTurn(rs.getString("role"), rs.getString("message")),
                reportId1, reportId2);
    }
