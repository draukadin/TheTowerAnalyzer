...
set CREDENTIALS_FILE=%APP_DIR%\credentials.json
...
if not exist "%USER_PROPERTIES_FILE%" (
    ...
    (
        echo # Google Drive / Sheets credential file paths
        echo # credentials.json      = service account key (used by Google Sheets^)
        echo # oauth-credentials.json = OAuth 2.0 client secret (used by Google Drive^)
        echo drive.credentials-file=%APP_DIR%\credentials.json
...
if not exist "%CREDENTIALS_FILE%" (
    ...
    echo.
    echo This is a service account key used by the Google Sheets integration.
    echo To obtain it:
    echo   1. Go to https://console.cloud.google.com/
    echo   2. Open your project and go to IAM ^& Admin ^> Service Accounts
    echo   3. Create a service account and grant it Sheets access
    echo   4. Create a JSON key for the service account
    echo   5. Save the downloaded file as: %CREDENTIALS_FILE%
    pause
    exit /b 1
)
...
if not exist "%OAUTH_CREDENTIALS_FILE%" (
    ...
    echo.
    echo This is the OAuth 2.0 client secret used by the Google Drive integration.
    echo To obtain it:
    echo   1. Go to https://console.cloud.google.com/
